...
interface DropdownProps {
  /** Base SharePoint internal name (no suffix). If you pass ...Id/LookupId we will strip it. */
  id: string;
  displayName: string;
  options: Opt[];
...
export default function DropdownComponent(props: DropdownProps): JSX.Element {
  ...
  const isLookup: boolean =
    fieldType === 'lookup' || /(Lookup)?Id$/i.test(id);

  const isMulti: boolean = !!multiSelect || !!multiselect;

  // Context (keep external shapes as unknown to avoid leaking anys)
  const {
    FormData,
    GlobalFormData,
    FormMode,
  ...
  // Field-level disable/hide rules (keep external types as unknown)
  React.useEffect(() => {
    if (FormMode === 4) return;

    const formFieldProps: FormFieldsProps = {
      disableList: AllDisableFields as unknown,
      ...
      formStateData: FormData as unknown,
      ListColumns: listCols as unknown,
    } as FormFieldsProps;
  ...
  // Handle UI events (Fluent v9 types; fields in data may be undefined)
